...
species Individual {
	...
	// 3. Perubahan symptomp
	action change_symptomp {
	...
	// 6. Reflex economical
	reflex economical when: current_day = 7 and current_hour = 18 and live = 1 {
		if (is_employed){
		...
		if (property > 0){
			int x <- rnd(0.1,0.9); //Biaya hidup orang2 sesuai dengan gaya masing2
		...
		if (infection_period = 0) {
			...
			if (infection_period = incubation_period) {
				...
				if (symptomps = asymptomic){
					severity <- rnd_choice(asymptomic,mild,moderate,severe,deadly);
				} else if (symptomps = mild) {
					severity <- rnd_choice(mild,moderate,severe,deadly);
				} else if (symptomps = moderate) {
					severity <- rnd_choice(moderate,severe,deadly);
				}

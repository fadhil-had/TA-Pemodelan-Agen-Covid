...
global {
	...
	reflex travel_coming when : (current_hour = 20 and flip(proba_travel) and not lockdown) { //Jika jam 8 malam dan flip proba maka dibuat individual
		int num_family_traveler <- rnd(1,20);
		...
		count_today[current_day_of_week] <- length (Individual where each.stat_traveler = come);
	}
	...
	reflex update_data when: current_hour = 23 {
		...
		num_hospitalized <- population count (each.quarantine_status = "hospital");
		...
		hospitalized_today <- population count (each.quarantine_status = "hospital" and each.quarantine_period < 24);
		...
		if (travel_today div num_population > 0.1 and average_come div num_population > 0.1){
